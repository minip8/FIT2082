...
class NewHashBoost:
    ...
    def __init__(self, num_classes, num_pairs_per_hash=8, lr=0.1, max_num_hashes=100):
        ...
        self.buffer_residuals = np.zeros(
            (max_num_hashes, 2**num_pairs_per_hash, num_classes), dtype=np.float32
        )
        self.buffer_hessian = np.zeros(
            (max_num_hashes, 2**num_pairs_per_hash, num_classes), dtype=np.float32
        )

        self.logits0 = np.zeros(
            (max_num_hashes, 2**num_pairs_per_hash, num_classes), dtype=np.float32
        )  # logits, but derived from numerator and denominator buffers only

        self.feature_indices = np.zeros(
            (max_num_hashes, num_pairs_per_hash), dtype=np.int32
        )
        self.midpoints = np.zeros(
            (max_num_hashes, num_pairs_per_hash), dtype=np.float32
        )
...
def _predict_multi_all0(
    X, feature_indices, midpoints, num_rounds, num_classes, logits0
):
...
@njit(
    "void(f4[:,:],f4[:,:],f4[:,:],i4[:],f4[:],i4,f4[:,:],f4[:,:],f4,f4[:,:],f4[:,:],f4[:,:],b1[:])",
    fastmath=True,
    cache=True,
)
def _update_single0(
...
@njit(
    "void(f4[:,:],f4[:,:,:],f4[:,:,:],i4[:,:],f4[:,:],i4,f4[:,:],f4[:,:],f4,i4,f4[:,:,:])",
    fastmath=True,
    cache=True,
    parallel=True,
)
...
@njit(
    "Tuple((i4[:],f4[:]))(f4[:,:],i4[:],f4[:,:],f4[:,:],i4,f4[:,:],f4[:,:],f4,i4,f4[:,:],f4[:,:])",
    fastmath=True,
    cache=True,
)
def _fit_new_hash0(
    ...
    feature_indices, midpoints = _assign(
        X, _pair(Y=Y, order=order, num_pairs=num_pairs_per_hash)
    )
...
@njit(
    "void(f4[:,:],i4[:],f4[:,:,:],f4[:,:,:],i4[:,:],f4[:,:],i4,i4,f4,i4,f4[:,:,:])",
    fastmath=True,
    cache=True,
)
def _fit_batch0(
    ...
    hessian = (probabilities * (1 - probabilities) + np.float32(1e-3)).astype(
        np.float32
    )
    ...
    feature_indices[num_rounds], midpoints[num_rounds] = _fit_new_hash0(
        X=X,
        Y=Y,
