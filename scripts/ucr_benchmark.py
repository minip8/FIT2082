...
def warm_up(device: str) -> None:
    ...
    the graphs built here serve every dataset.
    """

    Z = torch.randn(64, 16, device=device)
    y = np.arange(64) % 2

    fit_hashboost(Z, y, Z, y, num_classes=2, rounds=4, device=device, compile=True)
...
def main() -> None:
    ...
    for i, name in enumerate(names, 1):
        ...
        for model in todo:
            ...
            write_results(out, {model: entry}, info)

            errors[model].append(entry["final"]["te"])
            seconds[model] += fit_seconds(entry)

            line += f" {model} {entry['final']['te']:.4f} ({fit_seconds(entry):.1f}s)"
    ...
    for model in models:
        if errors[model]:
            print(
                f"  {model:10s} mean {statistics.mean(errors[model]):.4f} over "
